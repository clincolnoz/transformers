...
class TorchHFMLMModel(object):
    ...
    def prepare_raw_dataset(self):
        ...
        if self.data_args.dataset_name is not None:
            self.raw_datasets_from_hf_dataset()
            # If no validation data is there, validation_split_percentage will be used to divide the dataset.
            if "validation" not in self.raw_datasets.keys():
                self.create_validation_set(
                    path=self.data_args.dataset_name, name=self.data_args.dataset_config_name
                    )
        else:
            extension, data_files = self.raw_datasets_from_files()
            # If no validation data is there, validation_split_percentage will be used to divide the dataset.
            if "validation" not in self.raw_datasets.keys():
                self.create_validation_set(
                    path=extension, data_files=data_files
                    )
    ...
    def load_hf_model_and_tokenizer(self):
        ...
        if self.model_args.config_name:
            ...
            if self.model_args.config_overrides is not None:
                logger.info(f"Overriding config: {self.model_args.config_overrides}")
    ...
    def define_trainer(self):
        ...
        self.trainer = Trainer(
            ...
            compute_metrics=self.compute_metrics if self.training_args.do_eval and not is_torch_tpu_available() else None,
    ...
    def post_training_actions(self):
        kwargs = {"finetuned_from": self.model_args.model_name_or_path, "tasks": "fill-mask"}
        if self.data_args.dataset_name is not None:
    ...
    def check_last_checkpoint(self):
    ...
    def create_validation_set(self, **kwargs):
    ...
    def raw_datasets_from_hf_dataset(self):
        """If starting from hf_dataset"""
    ...
    def raw_datasets_from_files(self):
        """If starting from raw text files"""
        data_files = {}
        ...
        if extension == "txt":
            extension = "text"
        if "validation" in data_files.keys():
            self.raw_datasets = load_dataset(
                extension,
